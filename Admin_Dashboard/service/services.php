...
switch ($action) {
    case 'view':
        ...
        $query = 'SELECT * FROM printing_services';
        $result = mysqli_query($conn, $query);

        echo "<div class='list-header'>
                            <h2> Printing Service List</h2>
                            <a href='services.php?action=addp' class ='button button-add'>Add Service</a>
                        </div>";
        if (isset($_SESSION['message'])) { ?>
                            <div class="alert alert-<?php echo $_SESSION['msg_type']; ?> alert-dismissible fade show" role="alert">
                                <?php echo $_SESSION['message']; ?>
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<td>
                                    <a href='services.php?action=editp&service_id=".$row['service_id']."' class= 'button button-edit'>Edit</a> |
                                    <a href='services.php?action=deletep&service_id=".$row['service_id']."' class = 'button button-delete' onclick='return confirm(\"Are you sure you want to delete?\")'>Delete</a>
                                  </td>";
            echo '</tr>';
        }
    ...
    case 'addp':
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $insertQuery = 'INSERT INTO printing_services (service_id, service_name, service_description, status, image) VALUES (?, ?, ?, ?, ?)';
            $insertStmt = $conn->prepare($insertQuery);
        ...
        ?>                        
                        ...
                        <h2>Add New Service</h2>
                        <form action="services.php?action=addp" method="POST" enctype="multipart/form-data"> 
                            <table>
                                <tr>
                                    <th>Service Name</th>
                                    <td><input type="text" name="service_name" required></td>
    ...
    case 'editp':
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $updateQuery = 'UPDATE printing_services SET service_name = ?, status = ?, service_description = ?, image = ? WHERE service_id = ?';
            $updateStmt = $conn->prepare($updateQuery);
            $updateStmt->bind_param('ssssi', $service_name, $status, $service_description, $imageURL, $service_id);
        ...
        $selectQuery = 'SELECT * FROM printing_services WHERE service_id = ?';
        $selectStmt = $conn->prepare($selectQuery);
        $selectStmt->bind_param('i', $service_id);
        $selectStmt->execute();
        $selectResult = $selectStmt->get_result();
    ...
    case 'deletep':
        // Delete service
        $service_id = $_GET['service_id'];
        $deleteQuery = 'DELETE FROM printing_services WHERE service_id = ?';
        $deleteStmt = $conn->prepare($deleteQuery);
        $deleteStmt->bind_param('i', $service_id);
        $deleteStmt->execute();
